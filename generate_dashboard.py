...
const notePlugin = {{
  ...
  afterDatasetsDraw(chart) {{
    ...
    firstMeta.data.forEach((pt, i) => {{
      ...
      // 라벨이 차트 영역 밖으로 잘리지 않도록 위치 보정
      const area = chart.chartArea;
      minY = Math.max(minY, area.top + 42);
      ...
      // 회전(-22.5도) 시 텍스트가 우측 위로 뻗음 → 우측 경계에서 잘리면 좌측 정렬로 전환
      const projW = tw * Math.cos(Math.PI / 8);
      let x = pt.x, align = 'center';
      if (x + projW / 2 > area.right) {{ align = 'right'; x = Math.min(x, area.right); }}
      if (x - projW / 2 < area.left)  {{ align = 'left';  x = Math.max(x, area.left); }}
      ctx.textAlign = align;
      ctx.translate(x, minY - 16);
