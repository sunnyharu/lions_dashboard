...
def build_html(data: list, news: list, digest: str, raw_products_off: list, raw_products_on: list, apps_script_url: str = "") -> str:
    ...
    off_last_date = max((r["date"] for r in raw_products_off if r["date"]), default="")
    on_last_date  = max((r["date"] for r in raw_products_on  if r["date"]), default="")
  ...
  <div class="chart-card" style="height:520px">
    ...
    <div style="display:flex;flex-direction:column;width:100%;height:calc(100% - 48px)">
      ...
      <div style="display:flex;align-items:center;gap:10px;height:16px;margin:0 0 0 44px">
        ...
        <span style="font-size:10px;color:#999;white-space:nowrap">〜 중위값 기준 축 절단 · 평시 (중위값 이하) 〜</span>
        <svg width="100%" height="12" preserveAspectRatio="none" viewBox="0 0 1200 12" style="flex:1">
          <path d="M0 6 Q 15 0, 30 6 T 60 6 T 90 6 T 120 6 T 150 6 T 180 6 T 210 6 T 240 6 T 270 6 T 300 6 T 330 6 T 360 6 T 390 6 T 420 6 T 450 6 T 480 6 T 510 6 T 540 6 T 570 6 T 600 6 T 630 6 T 660 6 T 690 6 T 720 6 T 750 6 T 780 6 T 810 6 T 840 6 T 870 6 T 900 6 T 930 6 T 960 6 T 990 6 T 1020 6 T 1050 6 T 1080 6 T 1110 6 T 1140 6 T 1170 6 T 1200 6"
                fill="none" stroke="#bbb" stroke-width="1.5"/>
        </svg>
  ...
  afterDatasetsDraw(chart) {{
    ...
    firstMeta.data.forEach((pt, i) => {{
      ...
      for (let d = 1; d < numDatasets; d++) {{
        ...
        if (m.data[i] && !m.hidden) minY = Math.min(minY, m.data[i].y);
      }}
      minY = Math.max(minY, area.top);
      minY = Math.min(minY, area.bottom);
      items.push({{ x: pt.x, ptY: minY, note }});
    }});
    if (!items.length) return;

    // 2) 3단 꼬리표 배치: 가까운 라벨끼리 겹치지 않도록 단(tier) 순환 + 가로 밀어내기
    ctx.save();
    ctx.font = 'bold 9px sans-serif';
    const TIERS = 3, ROW_H = 12, PAD = 8;
    const tierRight = new Array(TIERS).fill(-Infinity);  // 각 단의 마지막 라벨 우측 끝
    items.forEach(it => {{
      const text = it.note.length > 14 ? it.note.substring(0, 13) + '…' : it.note;
      const tw = ctx.measureText(text).width;
      // 겹치지 않는 단 찾기 (없으면 가장 여유 있는 단)
      let tier = -1;
      for (let t = 0; t < TIERS; t++) {{
        if (it.x - tw / 2 > tierRight[t] + PAD) {{ tier = t; break; }}
      }}
      if (tier < 0) tier = tierRight.indexOf(Math.min(...tierRight));
      // 라벨 x: 포인트 중심, 단 안에서 이전 라벨과 겹치면 우측으로 밀기 + 경계 보정
      let lx = Math.max(it.x, tierRight[tier] + PAD + tw / 2);
      lx = Math.max(lx, area.left + tw / 2);
      lx = Math.min(lx, area.right - tw / 2);
      const ly = area.top + 9 + tier * ROW_H;
      tierRight[tier] = lx + tw / 2;
      ...
      ctx.beginPath();
      ctx.moveTo(lx, ly + 3);
      ctx.lineTo(it.x, Math.max(it.ptY - 4, ly + 3));
      ctx.stroke();
      // 포인트 표식
      ctx.fillStyle = '#C8102E';
      ctx.beginPath();
      ctx.arc(it.x, Math.max(it.ptY - 4, ly + 3), 1.5, 0, Math.PI * 2);
      ctx.fill();
      // 라벨
      ...
      y: {{
        ...
        ticks: {{
          callback: v => {{
            ...
            return v >= 1e8 ? ((v/1e8) % 1 ? (v/1e8).toFixed(1) : (v/1e8).toFixed(0))+'억'
                            : (v/1e4).toLocaleString('ko-KR')+'만';
          }}
        }},
      }}
    }}
